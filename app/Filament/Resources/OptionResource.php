        ...
        return $form
            ->schema([
                Forms\Components\Section::make('إعدادات الفلترة')->schema([
                    Forms\Components\Select::make('job_id')->options(Job::whereNull('excel_id')->latest()->pluck('name', 'id'))->label('الوظيفة')
                        ->required()->afterStateUpdated(function($state,$set){
                        $job=Job::find($state);
                        $set('criteria',$job->filter);
                    })->reactive(),
